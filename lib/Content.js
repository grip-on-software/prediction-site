    ...
    display() {
        ...
        if (valueClass !== "") {
            ...
            if (valueCompare !== null) {
                ...
                compare.classed('is-hidden', false)
                    .attr(
...
class LinkContent extends Content {
    display() {
        const linkTemplate = this.element.attr('data-link-template');
        const linkValue = this.element.attr('data-link-value');
        if (this.context.data[linkValue] !== null &&
            ...
            if (url) {
                this.element.attr('href', url.replace(/\/\$organization/g,
