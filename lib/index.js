    ...
    if (config.branches_url) {
        axios.get(config.branches_url)
            .then(response => {
                ...
                d3.select('#branches-menu').attr('role', 'menu');
...
/* Add export buttons */
const makeExport = function(context) {
    const options = [
        {
            'name': 'api',
            'icon': ['fas', 'fa-server'],
    ...
    const button = d3.select('#export').selectAll('button')
        .data(options)
        .enter()
        .append('button')
        .classed('button tooltip has-tooltip-bottom is-link is-outlined', true)
        .attr(ATTR_TOOLTIP, d => locales.attribute('export-tooltip', d.name))
