...
const getUrl = function(params) {
    ...
    // For localhost-based combined data sets, we do not have the $organization
    // variable within the URL to replace, instead propagate the information
    if (orgBase === base) {
        url.searchParams.set("combined", combined ? "true" : "");
        url.searchParams.set("organization", org);
    }

    // Propagate selected language
