...
const addBranchConfiguration = function(descriptions, data, name, branch=null) {
    const labels = data.labels ? locales.message('branch-labels', [
        _.map(Array.isArray(data.labels) ? data.labels : [data.labels],
            label => locales.retrieve(descriptions, label)
        ).join(', ')
    ]) : "";
    if (branch === null) {
        // Structured data for dataset
        return locales.message('branch-dataset', [data.model,
            labels ? labels : locales.message('branch-dataset-no-labels'),
            name === "master" ? locales.message('branch-default') :
                locales.message('branch-same-name', [name])
        ]);
    }
    ...
    if (labels) {
        branch.append('span').text(labels);
    }
    ...
    if (name === "master") {
        ...
        const addName = branch => {
            branch.append('span')
                .classed('name', true)
                .text(d => locales.message('branch-same-name', [name]));
        };
        addName(branch);
...
const makeBranches = function(context, target=null) {
    if (!config.branches_url) {
        if (context.data && context.data.configuration) {
            makeStructuredData(context.data.configuration,
                context.localization.descriptions
            );
        }
        else {
            axios.get(apiUrl(branch, 'configuration'))
                .then(configuration => {
                    makeStructuredData(configuration.data,
                        context.localization.descriptions
                    );
                })
                .catch(error => {
                    makeStructuredData({}, context.localization.descriptions);
                });
        }
        return;
    }
        ...
        .then(response => {
            ...
            items.each(function(d) {
                axios.get(apiUrl(d.name, 'configuration'))
                    .then(configuration => {
                        addBranchConfiguration(context.localization.descriptions,
                            configuration.data, d.name, d3.select(this)
                        );
                        if (d.name === branch) {
                            makeStructuredData(configuration.data,
                                context.localization.descriptions
...
const makePage = function(projectList, organization, project, sprints, dataContext) {
    ...
    makeBranches(context, d3.select("#branches-target"));
...
const makeStructuredData = function(configuration, descriptions) {
    if (typeof window.writeStructuredData !== "function") {
        return;
    }
    const api = {
        '@type': 'WebAPI',
    ...
    if (config.openapi_url) {
        api.documentation = config.openapi_url;
    }
    const dataset = {
        '@type': 'Dataset',
        'name': addBranchConfiguration(descriptions, configuration, branch),
        'url': datasetUrl,
        'variableMeasured': _.map(configuration.features, feature => {
            return {
                '@type': 'PropertyValue',
                'name': feature,
                'description': locales.retrieve(descriptions, feature)
            };
        })
    };
    if (configuration.dataset_archive_url && combined && branch === 'master') {
        dataset.sameAs = configuration.dataset_archive_url;
    }
    if (configuration.min_date && configuration.max_date) {
        dataset.temporalCoverage = `${configuration.min_date}/${configuration.max_date}`;
    }
    const structuredData = {
        '@type': 'WebApplication',
        '@id': predictionUrl,
        '@language': locales.lang,
        'name': locales.message('prediction-title'),
        'inLanguage': _.keys(locales.specs),
    ...
    window.writeStructuredData(locales, "prediction", structuredData);
};
