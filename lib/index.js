...
let makeContent = function(context) {
    let displayValue = function(key, value, element, context) {
        const type = (_.find(predicateTypes,
            (predicate) => predicate[0](value, element)
        ) || [null, Content])[1];
                        ...
                        axios.get(apiUrl(d.name, 'configuration'))
                            .then(configuration => {
                                ...
                                branch.select('span.branch')
                                    .text(locales.attribute('models', data.model));
...
let makePage = function(projectList, project, data, descriptions, tags, units, links) {
    makeNavigation(projectList, project);
    makeBranches(descriptions, d3.select("#branches-target"));
    makeContent({
        data,
        fallback: {
            project: project,
...
if (projectUrl === '') {
    ...
    .then(axios.spread(function (predictionData, projectList) {
        ...
        .then(axios.spread(function (descriptions, tags, units, links) {
            makePage(projectList, project, data, descriptions.data, tags.data, units.data, links.data);
