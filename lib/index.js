...
if (location.hostname === "localhost") {
    ...
    predictionUrl = prefix.length > 0 ? `${prefix.join('/')}/` : predictionUrl;
    ...
    apiUrl = (branch, item) => `${predictionUrl}api/v1${branch === 'master' ? '' : '-' + branch}/${item}`;
...
let getUrl = function(project=null, branch=null, sprint=null) {
    ...
    const base = specialLinks[branch] || `${config.branch_url}/${branch}/`;
    const url = new URL(base.replace(/\$organization/g, organization) +
...
let makePage = function(projectList, project, sprints, context) {
    ...
    const fallback = {
        ...
        name: context.data.id === null ? "" : locales.message("sprint-view"),
        features: (values) => _.pick(values, context.data.configuration.features)
    };
    makeContent(_.assign({}, context, {
        fallback,
        ...
        axios.get(dataUrl).then(function(predictionData) {
            ...
            .then(axios.spread(function (descriptions, sources, tags, units, short_units) {
                ...
                axios.all([
                    ...
                    makePage(projects, project, sprints.data, {
                        data, localization,
                        links: links.data,
