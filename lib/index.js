...
if (location.hostname === "localhost") {
    if (project === 'index' || project === '' || project === null) {
        ...
        linksUrl = `data/${project}/links${sprint === null ? "" : "." + sprint}.json`;
    ...
    const parts = _.dropWhile(url.split('/'),
        (part, index) => baseParts.length > index && baseParts[index] === part
    );
    if (parts[parts.length - 2] === 'sprint') {
        sprint = parts.pop();
        parts.pop();
    }
...
let getUrl = function(project=null, branch=null, sprint=null) {
    const specialLinks = {
        null: config.navLink,
    ...
    const url = new URL(
        ...
        (sprint === null ? '' : config.sprintLink + sprint),
        document.location);
...
let makeBranches = function(descriptions, target=null) {
    if (config.branches_url) {
        const filter = new Function('branch',
            config.branches_filter || 'return true;'
        );
        const alter = new Function('branch',
            config.branches_alter || 'return branch;'
        );
        axios.get(config.branches_url)
            .then(response => {
                const branches = d3.select('#branches').selectAll('dropdown-item')
                    .data(_.map(
                        _.filter(response.data.jobs, d => filter(d.name)),
                        d => {
                            d.name = alter(d.name);
                            return d;
                        }
...
let makeSprints = function(data, project, sprints, container) {
    container.classed('is-hidden', sprints.length <= 1)
        .selectAll('li')
        .data(sprints)
        .enter()
        .append('li')
        .classed('is-active', d => _.isObject(d) ? d.sprint_id === data.sprint :
            d === data.sprint
        )
        .append('a')
        .attr('href', d => getUrl(project, null, d.sprint_id || d))
        .text(d => _.isObject(d) && d.sprint_id !== data.sprint ?
            locales.message('sprint-link-name', [d.sprint_num, d.name]) :
            locales.message('sprint-link', [d.sprint_num || d]));
};

let makePage = function(projectList, projectMeta, project, data, localization, sourceLinks, sprints) {
    makeNavigation(projectList, projectMeta, decodeURIComponent(project));
    makeBranches(localization.descriptions || {},
        d3.select("#branches-target")
    );
    makeSprints(data, project, sprints || [], d3.select('#sprints'));
    makeContent({
        data,
        fallback: {
            project: decodeURIComponent(project),
            name: data.id === null ? "" : locales.message("sprint-view"),
...
if (dataUrl === '') {
    ...
    .then(axios.spread(function (predictionData, projectList) {
        ...
        .then(axios.spread(function (meta, descriptions, sources, tags, units, short_units) {
            const localization = {
            ...
            axios.all([
                ...
                makePage(projects, meta.data, project, data, localization, {
                    links: links.data,
        ...
        .catch(function(error) {
            makePage(projects, [], project, data, {}, {
                links: {},
                sources: {}
            }, []);
        });
