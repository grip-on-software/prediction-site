...
class Content {
    ...
    display() {
        ...
        const configuration = this.element.attr('data-configuration');
        if (configuration && !this.context.data.configuration[configuration]) {
            valueElement = null;
        }

        console.log([localizationKey, configuration, this]);
        ...
        if (valueElement && !valueElement.empty()) {
            ...
            this.element.text(vsprintf(this.context.locales.retrieve(this.context.localization[localizationKey], key, '%s'), [this.value]));
        }
        else {
...
class TableContent extends ObjectContent {
    ...
    addSourceIcon(icons) {
        icons.append('a')
            ...
            .attr('target', '_blank')
            .attr('title', d => this.context.links[d] ? this.context.locales.message("source-link") : null)
            .classed('is-hidden', d => {
    ...
    addAssignmentIcon(icons) {
        ...
        icons.append('span')
            .attr('title', d => assignments[d] ? assignments[d].expression : null)
            .classed('is-hidden', d => {
                // only show the link if it is present
                if (!assignments[d]) {
                    return true;
