...
class TableContent extends ObjectContent {
    ...
    addSourceIcon(icons) {
        icons.append('a')
            ...
            .attr('href', d => this.context.links[d] ?
                this.context.links[d].source :
                this.context.sources[d] ? this.context.sources[d] : null
            )
            .attr('target', '_blank')
    ...
    getAssignment(assignment) {
        ...
        if (assignment.attributes) {
            ...
            return _.replace(assignment.expression,
                new RegExp(_.join(assignment.attributes, '|'), "g"),
                attribute => this.context.locales.retrieve(features, attribute)
            );
        }
